...
def create_basic_ring_mesh(prompt, index, output_dir, latent=None):
    ...
    try:
        ...
        return output_file
            ...
            # Load the trained CAP3D model
            if cap3d_model and os.path.exists(cap3d_model):
                logger.info(f"Loading trained CAP3D model from {cap3d_model}")

                # Try multiple approaches to load the model with PyTorch 2.6 compatibility
                model_loaded = False
                checkpoint = None
                
                # Approach 1: Try with weights_only=False (may be unsafe but works with older models)
                try:
                    checkpoint = torch.load(cap3d_model, map_location=device, weights_only=False)
                    model_loaded = True
                    logger.info("Successfully loaded model with weights_only=False")
                except Exception as e:
                    logger.warning(f"Error loading with weights_only=False: {str(e)}")
                ...
                if not model_loaded:
                    try:
                        # Create a dummy model and save its state dict
                        dummy_model = torch.nn.Linear(10, 10)
                        dummy_file = os.path.join(output_dir, "dummy_model.pt")
                        torch.save(dummy_model.state_dict(), dummy_file)
                        
                        # Now try to load our target model using the same approach
                        checkpoint = {'model_state_dict': torch.load(cap3d_model, map_location=device, weights_only=True)}
                        model_loaded = True
                        logger.info("Successfully loaded model state dict directly")
                    except Exception as e:
                        logger.warning(f"Error loading as state dict: {str(e)}")
                ...
                # Try to load the model weights with error handling
                try:
                    if 'model_state_dict' in checkpoint:
                        model.load_state_dict(checkpoint["model_state_dict"])
                        model.eval()
                        ...
                        logger.warning("No model_state_dict found in checkpoint, using uninitialized model")
                except Exception as e:
                    logger.warning(f"Error loading model weights: {str(e)}")
                    logger.info("Continuing with uninitialized model")
                
                # Set to eval mode regardless
                model.eval()
                ...
                for i, prompt in enumerate(prompts):
                    ...
                    encoded = torch.zeros(512, device=device)
                    for j, char in enumerate(prompt[:512]):
                        encoded[j] = ord(char) / 255.0  # Same encoding as in training

                    # Generate latent vector
                    with torch.no_grad():
                        latent = model(encoded.unsqueeze(0))

                    # If we have a Shap-E model, we could decode the latent to a mesh here
                    # For now, create a simple mesh with the latent vector encoded in the vertices
                    output_file = os.path.join(output_dir, f"generated_ring_{i+1}.obj")

                    # Create a mesh that's slightly more interesting than a cube
                    # Use some values from the latent to influence the shape
                    latent_np = latent.cpu().numpy()[0]
                    scale_factor = 1.0 + 0.5 * np.tanh(latent_np[:3].mean())

                    with open(output_file, "w") as f:
                        f.write(f"# Generated Ring from prompt: {prompt}\n")
                        f.write(f"# Generated with latent dimension: {latent_dim}\n")

                        # Create a more interesting shape based on latent values
                        # Basic cube vertices with some variation
                        for x in [-1, 1]:
                            for y in [-1, 1]:
                                for z in [-1, 1]:
                                    # Add some variation based on latent
                                    mod_x = (
                                        x
                                        * scale_factor
                                        * (1.0 + 0.2 * np.tanh(latent_np[0]))
                                    )
                                    mod_y = (
                                        y
                                        * scale_factor
                                        * (1.0 + 0.2 * np.tanh(latent_np[1]))
                                    )
                                    mod_z = (
                                        z
                                        * scale_factor
                                        * (1.0 + 0.2 * np.tanh(latent_np[2]))
                                    )
                                    f.write(f"v {mod_x:.6f} {mod_y:.6f} {mod_z:.6f}\n")
                    ...
                    # Use latent values to influence the metadata
                    material = "gold"
                    if latent_np[3] > 0.5:
                        material = "white gold"
                    elif latent_np[4] > 0.5:
                        material = "platinum"

                    stone = "diamond"
                    if latent_np[5] > 0.7:
                        stone = "sapphire"
                    elif latent_np[6] > 0.7:
                        stone = "emerald"
                    elif latent_np[7] > 0.7:
                        stone = "ruby"

                    metadata = {
                        "prompt": prompt,
                        "latent_sample": latent_np[
                            :10
                        ].tolist(),  # Store part of the latent for reference
                        "components": [
                            {"name": "Metal_Head", "material": material},
                            {"name": "Metal_Shank", "material": material},
                            {"name": "Head_Center_Stone", "material": stone},
                            {"name": "Head_Accent_Stone", "material": "diamond"},
                        ],
                    }
                ...
                logger.warning(
                    "No valid CAP3D model found. Falling back to simulation."
                )
                ...
                for i, prompt in enumerate(prompts):
                    ...
                    output_file = os.path.join(output_dir, f"generated_ring_{i+1}.obj")
                    with open(output_file, "w") as f:
                        f.write("# Generated Ring (Fallback)\n")
                        f.write("# Prompt: " + prompt + "\n")
            ...
            logger.error(f"Error in ring generation: {str(e)}")
            # Fallback to simple generation
            logger.info("Falling back to simple generation due to error")

            for i, prompt in enumerate(prompts):
                logger.info(f"Generating fallback ring {i+1}/{len(prompts)}: {prompt}")
